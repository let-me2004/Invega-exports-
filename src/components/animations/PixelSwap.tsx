...
type PatternType = 'random' | 'center' | 'edges' | 'left-to-right' | 'right-to-left' | 'top-to-bottom' | 'bottom-to-top' | 'diagonal' | 'spiral';
...
const buildKeyframes = ({ ease, startScale, endScale, spin, fade, mode }: any) => {
  const windowKfs = [];
  const contentKfs = [];
  ...
  for (let step = 0; step <= KEYFRAME_STEPS; step += 1) {
    ...
    let scale, angle, opacity;
    if (isShatter) {
      scale = endScale - (endScale - startScale) * eased;
      angle = spin * eased;
      opacity = fade ? (1 - eased) : 1;
    } else {
...
export default function PixelSwap({
  ...
  const config = { duration, pixelDuration, pixelSpin, pixelScale, pixelRadius, fade, easing, onComplete, mode };
  ...
  useEffect(() => {
    ...
    const finish = () => {
      if (settings.mode === 'shatter' && layerRefs.current[settings.mode === 'shatter' ? (to ? 0 : 1) : (to ? 1 : 0)]) {
        layerRefs.current[settings.mode === 'shatter' ? (to ? 0 : 1) : (to ? 1 : 0)]!.style.visibility = ''; // Restore visibility
      }
      stopAnimations();
      setShownActive(to);
      setTransition(null);
    ...
    const sourceIndex = settings.mode === 'shatter' ? (to ? 0 : 1) : (to ? 1 : 0);
    ...
    // Hide the source layer ONLY after pixels are safely generated and animations attached
    if (settings.mode === 'shatter') {
      source.style.visibility = 'hidden';
    }

    timerRef.current = window.setTimeout(finish, total);
    return () => {
      if (settings.mode === 'shatter' && source) source.style.visibility = '';
      stopAnimations();
    };
  ...
  const renderLayer = (content: ReactNode, index: number) => {
    ...
    // Use the original ReactBits logic to prevent React-driven flashes
